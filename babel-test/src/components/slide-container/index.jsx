...
export default class WgSlideContainer extends React.PureComponent {
    ...
    render() {
        ...
        /**气泡 start */
        let { isOpened, top, left, zindex, proportion, isBgcColor, isFlexEnd } = this.state;
        ...
        let itemstyle = { height };
        let historyItem = { height: `${height}px`, justifyContent: isFlexEnd ? 'flex-end' : '', overflow: '' };

        //console.log('top, left:', top, left);
        /**气泡 end */
        return (
            <View className='wg-slide-container' style={itemstyle}>
                <View className='wg-history-item' style={historyItem}
                    ...
                    >
                        ...
                        {!hideLastLine && <View className={classNames(lineRightOrCenter === 'center' ? 'wg-show-line' : 'wg-show-line1')} ></View>}
                    </View>

                    <View className='wg-item-delete wg-right' style={itemStyle} >
                        {
                            itemsList.map((item, index) => {
                                // console.log('item.style', item.style);
                                return <View className='wg-history-btn wg-overhead' key={item.text} taroKey={item.text}
                                    style={{...item.style, ...style}} onClick={item.onClick}>{item.text}</View>;
                            })
                        }
                    </View>
                </View>
                <WgBubble ref={this.refCat} isOpened={isOpened} isArrowUp={indexState} items={itemsList} customStyle={customStyle} arrowStyle={arrowStyle}></WgBubble>
            </View>
        );
    }
}

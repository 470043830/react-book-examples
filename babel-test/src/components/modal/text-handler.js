...
function getTextListByHighWord(content, highWord, highStyle) {
    ...
    if (content) {
        ...
        for (let index = 0; index < splits.length; index++) {
            textList.push({ key: 'id_' + key++, text: splits[index], style: '' });
            if (index < splits.length - 1) {
                textList.push({ key: 'id_' + key++, text: split_str, style: highStyle ? highStyle : 'color:#49C167;' });
            }
        }
    }
    return textList;
...
function covertTextArray(textArray, word) {
    ...
    // console.log('covertTextArray: ', textArray, word);

...
export default class WgFoldText extends WgComponent {
    constructor(props) {
        ...
        this.onUnFoldText = () => {
            ...
            // console.log('onUnFoldText...', tipText, this);
            if (!tipText) {
                return;
            }
    ...
    async checkFold() {
        let res = await this.getBoundingRects();
        // console.log('---WgFoldText---checkFold:', res, this);
    ...
    handleText() {
        const { content, highWord, textStyle } = this.props;
        const alltext = [];

        if (!highWord) {
            alltext.push({ text: content, style: textStyle || 'color:#2E2E2F;' });
        } else {
            const split_str = '' + highWord;
            const splits = content.split(split_str);
            let key = 0;

            // console.log('---WgFoldText---handleText: ', splits.length, split_str, splits);
            for (let index = 0; index < splits.length; index++) {
                alltext.push({ key: 'id_' + key++, text: splits[index], style: textStyle || 'color:#2E2E2F;' });
                if (index < splits.length - 1) {
                    alltext.push({ key: 'id_' + key++, text: split_str, style: 'color:#49C167;' });
                }
            }
        }
    ...
    render() {
        const { content, lineClamp, isNeedUnfold, onTextClick } = this.props;
        const { curLineClamp, tipText, isTextTested } = this.state;
        ...
        return (
            <View className='wg-fold-box' id={`wg-fold-box-id-${WgFoldTextId}`}>
                <View id={this.queryIds[1]} className='wg-fold-box__content' style={multiTextClampStyle} onClick={onTextClick}>
                    {
                        allTextItems.map((item) => (<Text key={item.key} style={item.style}>{item.text}</Text>))
                    }
                </View>
                {isNeedUnfold && !isTextTested &&
                    <View id={this.queryIds[0]} className='wg-fold-box__test'>
                        {content}
                    </View>
                }
                ...
                <View className='wg-fold-box__tip' onClick={this.onUnFoldText} >{tipText}</View>
            </View>
        );
    }
}
